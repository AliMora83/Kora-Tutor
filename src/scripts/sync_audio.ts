...
if (!admin.apps.length) {
    admin.initializeApp({
        // Uses Application Default Credentials — no service-account.json needed.
        // Run `gcloud auth application-default login` once if not already set up.
...
async function main() {
    ...
    const geminiFiles: Array<{ name: string; uri: string; mimeType: string }> = [];
...
main().catch((err) => {
    console.error('❌ sync_audio failed:', err);
    process.exit(1);
});
