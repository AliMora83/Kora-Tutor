  ...
  useEffect(() => {
    ...
    async function fetchUrl() {
      try {
        ...
        let audioRef;
        if (decodedFilename.startsWith('training_audio/')) {
            audioRef = ref(storage, decodedFilename);
        } else {
             audioRef = ref(storage, `training_audio/${decodedFilename}`);
        }
  ...
  if (error) {
    return (
      <div className="flex items-center gap-3 p-3 bg-red-900/20 rounded-xl border border-red-500/50 text-red-400">
        ...
        <span className="text-sm">Failed to load {filename}</span>
      </div>
    );
  }
  ...
  return (
    <div className="my-2 p-3 bg-[#1a1a1a] hover:bg-[#222222] transition-colors rounded-xl border border-[#3a3a3a] flex flex-col gap-2">
      <span className="text-sm font-semibold text-secondary flex items-center gap-2">
        ...
        {decodeURIComponent(filename).replace(/\.mp3$/i, '').replace('training_audio/', '')}
      </span>
